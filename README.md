...
CAS 已集成 `Codex-5.5-codex-instruct-5.5` 的 GPT-5.5 指令注入方式：

- “一键破甲”：先备份 `config.toml`，再在 Codex 根目录写入 `gpt5.5-unrestricted.md`，并设置 `model_instructions_file = "./gpt5.5-unrestricted.md"`。
- “一键还原”：先备份 `config.toml`，恢复破甲前的 `model_instructions_file` 配置；如果破甲前没有该配置，则删除这一行，同时删除 CAS 写入的 `gpt5.5-unrestricted.md`。

操作前请彻底退出 Codex，操作后重新打开 Codex 生效。
...
- `config.toml` 中的 `model_instructions_file`（仅在使用一键破甲/还原时）
- Codex 根目录下的 `gpt5.5-unrestricted.md`（仅在使用一键破甲/还原时）

...
- `src/CodexApiSwitcher/` 是 Avalonia 跨平台主实现；macOS 使用 Keychain 保存 Key、LaunchAgent 开机启动、菜单栏驻留和文件独占锁实现可靠单实例。
- `.app` 的 `CFBundleExecutable` 直接指向 Mach-O 主程序，避免 shell 入口在新版 LaunchServices 下触发 `kLSNoExecutableErr`。
- `work/build-cross-platform.sh` 会构建 arm64/x64、生成 icns、校验 plist、ad-hoc 签名并用 `ditto` 生成保留执行权限的 zip。
- `work/test-macos.sh` 覆盖 Keychain/凭据助手、双向切换、SQLite/JSONL 同步、LaunchAgent、UI 主要按钮和第二次启动唤醒；设置 `CAS_TEST_REAL_KEYCHAIN=1` 使用真实系统 Keychain 并在测试结束后清理测试项。

- EXE 只允许修改所选根目录内的 `config.toml`、`config-switcher-backups`、`api-switcher`、`history_sync_backups`、根目录 `state_5.sqlite`、`sqlite/state_5.sqlite` 和 CAS 写入的 `gpt5.5-unrestricted.md`。
...
- 一键破甲/还原只修改 `model_instructions_file` 和 `gpt5.5-unrestricted.md`，不会改动 `auth.json`、会话正文、记忆或 MCP 配置。
